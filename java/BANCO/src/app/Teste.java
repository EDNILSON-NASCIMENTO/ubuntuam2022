...
public class Teste {
	...
	public static void main(String[] args) {
		...
		Empresa cemp1 = new Empresa(897,10000.00);
		...
		do {
			...
			if(tipoConta == '1') {
				...
				System.out.println("MOVIMENTAÇÃO BANCARIA - CONTA EMPRESA");
				for(int x=1; x<=10; x++) {
					System.out.println("SALDO CONTA R$ "+cemp1.getSaldo()+" Limite atual :"+cemp1.getEmprestimoEmpresa());
					System.out.println("MOVIMENTO :"+x);
					System.out.print("[D]ébito ou [C]rédito :");
					tipo = leia.next().toUpperCase().charAt(0);
					System.out.print("Qual o valor R$ : ");
					valor = leia.nextDouble();
					if(tipo=='D') {
						if(valor>cemp1.getSaldo() && valor <= cemp1.getSaldo()+cemp1.getEmprestimoEmpresa()) {
							System.out.println("Gostaria de solicitar um emprestimo ? S/N :");
							char opEmp = leia.next().toUpperCase().charAt(0);
							if(opEmp=='S') {
								System.out.println("Socilicite o emprestimo até o limide de "+cemp1.getEmprestimoEmpresa()+" - R$  :");
								double valorEmpAtual = leia.nextDouble();
								cemp1.pedirEmprestimo(valorEmpAtual);
							}
						}
						cemp1.debito(valor);
				...
				if(cemp1.getEmprestimoEmpresa()>0.00) {
					System.out.println("Saldo atual R$ : "+cemp1.getSaldo()+" - Emprestimo R$ "+cemp1.getEmprestimoEmpresa());
					if(cemp1.getEmprestimoEmpresa()>1000) {
						System.out.print("Gostaria de solicitar um emprestimo? S/N : ");
						char opteste = leia.next().toUpperCase().charAt(0);
						if (opteste=='S') {
							System.out.print("Valor solicitado R$ : ");
							double valorEmprestimo = leia.nextDouble();
							cemp1.pedirEmprestimo(valorEmprestimo);
						}
					}
				}
				
				System.out.println("Saldo final R$ : "+cemp1.getSaldo()+" - Emprestimo atual R$ :"+cemp1.getEmprestimoEmpresa());
			}
